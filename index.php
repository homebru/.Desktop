...
function buildHtml($blockArray, $orange) {
    ...
    foreach($blockArray as $key => $subArray) {
        ...
        if($key === 'comment') {
            $html .= "<blockquote class=\"blockquote\"><p>{$subArray}</p><input type=\"hidden\" name=\"#{$id}_#comment\" value=\"".htmlentities($subArray)."\"></blockquote>";
            continue;
        }
		...
		$html .= "<div class=\"panel panel-primary\" style=\"border-color:{$theOrange};\">
                      <div class=\"panel-heading\" style=\"background-color:{$theOrange};border-color:{$theOrange};\">
                        ...
                        <div class=\"col-md-1\">
                            <button type=\"button\" class=\"btn btn-addnew\" aria-label=\"Add New Item\" title=\"Add New Item\"><span class=\"glyphicon glyphicon-plus\"></span></button>
                        </div>
				      </div>
        ...
        foreach($subArray as $name => $val) {
			...
			if($name[0] === 'c') {
				$html .= "<blockquote class=\"blockquote-reverse\"><p>{$val}</p><input type=\"hidden\" name=\"#{$id}_#comment\" value=\"{$val}\"></blockquote>";
                continue;
            }
    ...
    <form class="container">
        <div class="col-md-12" id="header">
            ...
            <div class="col-md-10" id="browser">
                ...
                <button class="btn btn-save pushed-right hide" type="button" value="New" id="new-section" name="new-section" title="Add New Section"><span class="glyphicon glyphicon-list"></span></button>
                <button class="btn btn-save pushed-right" type="button" value="Save" id="save" name="save" title="Save Changes"><span class="glyphicon glyphicon-save-file"></span> Save</button>
            </div>
